...
impl From<Request> for FilePath {
    fn from(req: Request) -> Self {
        Self(req.param("name").unwrap().into())
    }
}

#[get("/drive/file_hints")]
#[mime("application/json")]
pub async fn file_hints(f: FilePath) -> Vec<u8> {
    let hints = FileHints::new(f);
    println!("{:#?}", hints);
...
impl FileHints {
    fn new(p: FilePath) -> Self {
        let f = File::open(&p.0).unwrap();
        let m = f.metadata().unwrap();

        let kind = hints::kind(&m);
        let size = hints::size(&m, kind);
        let count = hints::count(&p.0, kind);
        let ext = hints::extension(&p.0, kind);
